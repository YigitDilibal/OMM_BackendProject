Feature: As a provider, I want to be able to access the shops via the API connection.
  ...
  response_message “Blogs Listed Successfully”, and blog id(10) includes all expected blog fields.
    ...
    And The api user validates the "<shop_code>", "<shop_name>", "<country_code>", "<tax_allow>", "<tax_number>", "<contact_no>" contents of the data in the response body.
    And The api user validates the "<email>", "<address>", "<country_name>", "<state_name>", "<city_name>","<postal_code>" contents of the data in the response body.
